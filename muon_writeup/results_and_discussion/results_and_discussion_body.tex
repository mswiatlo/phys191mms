...
where $h$ is the number of bins, $IQR$ is the interquartile range, and $N$ is the number of data points in the set.  According to this formula, a good binwidth for our data is $227$ ns.  Because there does not exist a formula for optimal binwidth (the goodness of a specific binwidth is dependent upon the distribution of the data), the comparable binwidth of $200$ ns is a valid, much more convenient choice. 
...
\caption{\small{A histogram of muon lifetimes. The lifetimes follow exponential decay to some background noise level.  The data was fit using 2-parameter nonlinear regression using $\sqrt{N}$ weighting and excluding the first $2.0~\mu$s.  The fit has an $r^{2}$ value of $0.996$ and produces a $\tau_{\mu}$ of $2.12 \pm 0.06~\mu$s.}}
\label{fig:muondecay}
\end{center}
\end{figure}
...
\caption{\small{This plot shows the effect of excluding data up to some cutoff time $t_{c}$ from the fitting process.  We chose $t_{c}$ to be $2.0~\mu$s because it produced a value for $\tau_{\mu}$ that was in a range of relative stability and because it produced a fit with an $r^{2}$ value $> 0.995$ as seen above.}}
\label{fig:rsq}
\end{center}
\end{figure}
...
all three scintillator panels.  More than $50,000$ events were observed in
...
\vspace{-2mm}
\caption{\small{These raw pulse heights produced by the PMTs correspond to muon and electron energies deposited in the scintillators.  Data from (a) is used to calibrate a scale from pulse height (measured in V) to energy (measured in MeV).  Data from (b) was used to determine the electron cutoff energy.  Together information from both plots were used to calculate the muon mass.}
...
recorded for roughly five days and included nearly $7,000$ events.  The
...
see (relative to the number of events observed, which was nearly 7,000)
for some $V_{e}$.
